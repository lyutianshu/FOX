...
namespace scd {
	...
	uint32_t	CGraph::RemoveEdgesNoTriangles( uint32_t numThreads) {
		uint32_t* tAdj = new uint32_t[sizeof(m_Adjacencies) / sizeof(m_Adjacencies[0])];
		memcpy(tAdj,m_Adjacencies, sizeof(m_Adjacencies) / sizeof(m_Adjacencies[0]));	
		Node* tNode = new Node[sizeof(m_Nodes) / sizeof(m_Nodes[0])];
		memcpy(tNode,m_Nodes, sizeof(m_Nodes) / sizeof(m_Nodes[0]));
...
//			uint32_t auxPossibleTriangles = newDegree*(newDegree - 1);
...
//		m_NumEdges-=(numEdgesRemoved/2);
		memcpy(m_Adjacencies,tAdj, sizeof(m_Adjacencies) / sizeof(m_Adjacencies[0]));	
		memcpy(m_Nodes,tNode, sizeof(m_Nodes) / sizeof(m_Nodes[0]));
		printf("new m_NumEdges is:%d",m_NumEdges);
		return 0;
	}
}
